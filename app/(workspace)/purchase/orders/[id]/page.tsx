...
export default async function PurchaseOrderDetailPage({
  ...
  let order;
  try {
    order = await purchaseOrderService.findById(params.id);
  } catch {
    notFound();
  }
  ...
  async function confirmOrder() {
    "use server";
    await purchaseOrderService.confirm(params.id);
    revalidatePath(`/purchase/orders/${params.id}`);
    ...
    <div className="space-y-6 max-w-5xl">
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            {order.status === "CONFIRMED" && (
              <Link href={`/purchase/bills/new?poId=${order.id}`}>
                <Button size="sm" className="gap-1.5 bg-emerald-600 hover:bg-emerald-700">
                  <FilePlus className="h-4 w-4" /> Create Vendor Bill
                </Button>
              </Link>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-2">
          <CardHeader>
            <CardTitle className="text-base font-semibold">Ordered Line Items</CardTitle>
          </CardHeader>
          <CardContent>
