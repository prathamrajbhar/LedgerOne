...
export class S3StorageClient {
  ...
  constructor() {
    this.bucketName = process.env.AWS_S3_BUCKET_NAME || "dummy-bucket";
  }

  private getS3(): AWS.S3 {
    if (!this.s3) {
      const accessKeyId = process.env.AWS_ACCESS_KEY_ID || "dummy_access_key";
      const secretAccessKey = process.env.AWS_SECRET_ACCESS_KEY || "dummy_secret_key";
